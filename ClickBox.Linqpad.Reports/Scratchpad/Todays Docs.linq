<Query Kind="Statements">
  <Connection>
    <ID>898042e4-f676-4c29-8008-4d76cbad8f62</ID>
    <Persist>true</Persist>
    <Driver Assembly="Madd0.AzureStorageDriver" PublicKeyToken="47842961fb3025d7">Madd0.AzureStorageDriver.AzureDriver</Driver>
    <DriverData>
      <UseHttps>true</UseHttps>
      <NumberOfRows>100</NumberOfRows>
      <AccountName>clickbox</AccountName>
      <AccountKey></AccountKey>
    </DriverData>
  </Connection>
</Query>

MonthlyDocuments.ToArray().Where (md =>  md.Timestamp.ToLocalTime().DateTime.ToString().Substring(0,10) == DateTime.Today.ToString().Substring(0,10)).Select (md => new {
 md.PartitionKey,
 md.Timestamp.ToLocalTime().DateTime,
 md.CompanyName
}).OrderBy (md => md.DateTime.ToLocalTime()).Dump();